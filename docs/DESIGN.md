...
`src/renderer/src/index.css` is the only place raw colour values live. The
palette sits on `:root` as plain custom properties, and Tailwind's `@theme
inline` block maps every design token onto one of them:
...
**Adding a theme** therefore means adding one block — no component changes:
